...
class GlobalLogic {
  ...
  Future getCurrentLanguageCode() async {
    final list =
        await SharedUtil.getStringList(Keys.currentLanguageCode);
    if (list == null) return;
  ...
  Future getCurrentLanguage() async {
    final currentLanguage =
        await SharedUtil.getString(Keys.currentLanguage);
  ...
  Future getAppName() async {
    final appName = await SharedUtil.getString(Keys.appName);
  ...
  Future getNickName() async {
    final nickName = await SharedUtil.getString(Keys.nickName);
  ...
  Future getFaceUrl() async {
    final faceUrl = await SharedUtil.getString(Keys.faceUrl);
  ...
  Future getGender() async {
    final gender = await SharedUtil.getInt(Keys.gender);
  ...
  Future getLoginState() async {
    final hasLogged = await SharedUtil.getBoolean(Keys.hasLogged);
    if (hasLogged == null)
  ...
  Future getAccount() async {
    final appAccount = await SharedUtil.getString(Keys.account);
