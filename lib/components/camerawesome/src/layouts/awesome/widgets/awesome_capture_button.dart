...
import 'package:gardener/components/camerawesome/src/orchestrator/analysis/analysis_controller.dart';
import 'package:gardener/components/camerawesome/src/orchestrator/models/media_capture.dart';
  ...
  get onTap => () {
        widget.state.when(
          onPhotoMode: (photoState) async {
            String? filepath = await photoState.takePhoto();
            widget.onPhotoCreated(filepath);
          },
          onVideoMode: (videoState) => videoState.startRecording(),
          onVideoRecordingMode: (videoState) async {
            String? filepath = await videoState.stopRecording();
            widget.onVideoCreated(filepath);
            return filepath;
          },
        );
      };
